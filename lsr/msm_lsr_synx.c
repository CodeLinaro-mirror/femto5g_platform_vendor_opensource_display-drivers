...
int lsr_create_reusable_hsynx(u32 *h_synx_arr)
{
	...
	if (!dev || !dev->hwfence_data.hw_fence_handle) {
		dprintk(LSR_ERR, "invalid device or hw_fence_handle\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < LSR_REUSABLE_FENCE_MAX; i++) {
		import_params.indv_v2.new_h_synx = &h_synx_arr[i];
		...
		if (ret) {
			dprintk(LSR_ERR, "failed to create reusable hw fence[%d]: %d\n", i, ret);
			return ret;
		}

		dev->lsr_reusable_hsynx[i] = h_synx_arr[i];
		dprintk(LSR_SYNX, "created reusable hfence[%d]: 0x%x\n", i, h_synx_arr[i]);
	}
...
static int lsr_sess_deinit_synx_v2(struct lsr_device *dev)
{
	...
	for (i = 0; i < LSR_REUSABLE_FENCE_MAX; i++) {
		if (dev->lsr_reusable_hsynx[i]) {
			ret = synx_release(dev->hwfence_data.hw_fence_handle,
					dev->lsr_reusable_hsynx[i]);
			if (ret)
				dprintk(LSR_ERR, "Failed to release reusable hw fence[%d]\n", i);
		}
	}
