...
#if IS_ENABLED(CONFIG_DRM_SDE_SHD) /*enable CONFIG_DRM_SDE_SHD*/
...
static void _sde_rm_populate_dp_lm_mask(struct sde_rm_state *state,
		struct drm_connector *conn)
{
	struct sde_connector *c_conn = NULL;
	struct sde_rm_hw_blk *blk;
		...
		struct drm_connector_state *conn_state)
{
	...
	_sde_rm_populate_dp_lm_mask(state, conn_state->connector);
...
#else  /*disable CONFIG_DRM_SDE_SHD*/
