		...
		struct sde_crtc_state *cstate)
{
	...
	for (i = 0; i < num_misrs; ++i) {
		...
		for (j = 0; j < ROI_MISR_MAX_ROIS_PER_MISR; ++j) {
			if (!(l_dspp_hw_cfg->roi_mask & BIT(j)))
				continue;
...
void sde_roi_misr_setup(struct drm_crtc *crtc)
{
	...
	for (i = 0; i < sde_crtc->num_mixers; ++i) {
		...
		if (hw_dspp && (hw_dspp->cap->features & BIT(SDE_DSPP_ROI_MISR_BYPASS))) {
			hw_dspp->ops.setup_roi_misr(hw_dspp,
					misr_hw_cfg->dspp_roi_mask,
					misr_hw_cfg->dspp_roi_rect);

			if (hw_ctl->caps->features & BIT(SDE_CTL_UNIFIED_DSPP_FLUSH))
				hw_ctl->ops.update_bitmask_dspp_subblk(hw_ctl,
						hw_dspp->idx,SDE_DSPP_ROI_MISR_BYPASS, true);
			else
