...
static int shd_parse_base(struct drm_device *drm_dev, struct shd_display_base *base)
{
	...
	rc = of_property_read_u16(node, "qcom,mode-h-front-porch", &h_front_porch);
	if (rc) {
		SDE_ERROR("failed to read h-front-porch, rc=%d\n", rc);
		goto fail;
	}

	rc = of_property_read_u16(node, "qcom,mode-h-pulse-width", &h_pulse_width);
	if (rc) {
		SDE_ERROR("failed to read h-pulse-width, rc=%d\n", rc);
		goto fail;
	}

	rc = of_property_read_u16(node, "qcom,mode-h-back-porch", &h_back_porch);
	if (rc) {
	...
	rc = of_property_read_u16(node, "qcom,mode-v-front-porch", &v_front_porch);
	if (rc) {
		SDE_ERROR("failed to read v-front-porch, rc=%d\n", rc);
		goto fail;
	}

	rc = of_property_read_u16(node, "qcom,mode-v-pulse-width", &v_pulse_width);
	if (rc) {
		SDE_ERROR("failed to read v-pulse-width, rc=%d\n", rc);
		goto fail;
	}

	rc = of_property_read_u16(node, "qcom,mode-v-back-porch", &v_back_porch);
	if (rc) {
